...
#
# This program explores the use of random dimensions as a starting point.
#
...
#Opening the .csv to write to and writing the headers for each column
fileName = 'Random.csv' 
...
#Looping through 50 random designs
for i in range(0, 50):
    i = i + 1
    #Assigning random numbers 
    h = random() * (hLmax - hLmin) + hLmin
    f1 = random() * (fd1max - fd1min) + fd1min
    f2 = random() * (fd2max - fd2min) + fd2min
    th = random() * (tmax - tmin) + tmin
    
    #Calculated values temp h
    FirstHole = ((h - h0) / L * xFirstHole + h0)
    hSecondHole = ((h - h0) / L * xSecondHole + h0)
    d1 = f1 * hFirstHole d2 = f2 * hSecondHole
    tao = F / (th * h)
    Mdb()
    ...
    p = mdb.models['Model-1'].parts['Part-1'] 
    f = p.faces faces = f.getSequenceFromMask(mask=('[#1 ]', ), ) 
    ...
    p = mdb.models['Model-1'].parts['Part-1']
    f = p.faces 
    faces = f.getSequenceFromMask(mask=('[#1 ]', ), ) pickedRegions =(faces, ) p.setElementType(regions=pickedRegions, elemTypes=(elemType1, elemType2))
    ...
    odbFile = ModelName + '.odb' maxMisesStress = getMaxMises(odbFile, '') 
    ...
    DataFile.write('%f, %f, %f, %f, %f, %f, %f, %f\n'%(i, h, f1, f2, th, maxMisesStress, mass, maxMisesStress / mass)) 
    DataFile.close(