...
#
# This program explores the use of a full factorial of dimensions as a starting point.
#
...
#Looping throught the design constraints
for h in hL: 
    for f1 in fd1: 
        for f2 in fd2: 
            for th in t:
            ...
            p = mdb.models['Model-1'].parts['Part-1'] 
            f = p.faces
            faces = f.getSequenceFromMask(mask=('[#1 ]', ), ) region = p.Set(faces=faces, name='Set-1') p = mdb.models['Model-1'].parts['Part-1'] p.SectionAssignment(region=region, sectionName='Section-1', offset=0.0, offsetType=MIDDLE_SURFACE, offsetField='', thicknessAssignment=FROM_SECTION)
